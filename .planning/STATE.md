...
**Current focus:** Phase 11 (re-opened) — gap closure. Wave 1: 11-09 ✓ + 11-11 ✓ + 11-14 ✓. Wave 2: 11-12 (G-2) PARTIAL — Path A feature-gate MECHANISM landed + cargo-tree-proven (commit f04e2095dc); the umbrella's 280 kernel deps are now optional behind per-family features so a single-family build pulls only that family's kernels (the memory-safe oracle path). REMAINING in 11-12 (deferred, heavy-build session): umbrella source cfg-gating + harness repair + f32 sweeps (see 11-12-ORACLE-F32-LOG.md). 11-10 ○ deferred. 11-13 (G-5 closure) blocked on 11-10+11-12. Phase NOT complete (3 plans remain).
...
Wave 2 status: 11-12 G-2 🟡 PARTIAL (commit f04e2095dc) — Path A feature-gate MECHANISM landed + cargo-tree-proven: 280 umbrella kernel deps → optional behind oracle-{lda,gga,mgga} features (default=all → 11-14 default build preserved by construction); single-family build resolves only that family's kernels (lda43/gga131/mgga106, no cross-family). NO build run; .cargo/config.toml untouched. DEFERRED w/ turnkey spec (11-12-ORACLE-F32-LOG.md): umbrella source cfg-gating (via generators + hand-gate evaluate.rs router; cheap check = `cargo check -p libxc_rs --lib --no-default-features` = 0 kernels), stale-harness repair (libxc_kernel_math::deferred::{lda,mgga}::is_deferred), f32 env-gate wiring, paced per-family f32 sweeps (Task 2) + tolerance checkpoint (Task 3) → then 11-12-SUMMARY.md. WHY: umbrella un-cargo-checkable cheaply (default=all 280=OOM); blind source-cfg under deny(warnings) risks 11-14 regression, so it rides the heavy-build session.
Remaining: 11-12 finish (source-cfg + sweeps, heavy), 11-13 (G-5, W3, depends 11-10+11-12). Original 11-01..08 untouched (all have SUMMARYs).
