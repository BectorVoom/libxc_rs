...
Last activity: 2026-05-20 - Completed quick task 260520-eem: mgga_c_tpssloc sub-crate split (Option A) — SUCCESS. `cargo build -p libxc-kernel-mgga_c_tpssloc` now compiles under jobs=1 (~9 GB peak) via a facade + 7 shard sub-crates. The tpssloc OOM (open since 260520-a0c) is fully resolved.
...
Last session: 2026-05-20 (quick task 260520-eem — Option A sub-crate split)
Stopped at: mgga_c_tpssloc OOM FULLY RESOLVED. The two-layer fix is complete: (1) hierarchical CSE (260520-c91) bounded per-#[cube]-fn proc-macro RAM; (2) sub-crate split (260520-eem) bounded aggregate rustc state. `cargo build -p libxc-kernel-mgga_c_tpssloc` now compiles under jobs=1 at ~9 GB peak via a facade (keeps the public crate name) + 7 `_pK` shard sub-crates. New reusable tool `tools/split_per_functional_subcrate.py`. KEY LEARNING: per-rustc RSS scales with PART COUNT not file count (budget future splits ≲70 parts/shard). NEXT candidate steps (none blocking, none executed): (a) re-add libxc-kernel-mgga_c_tpssloc to workspace default-members now that it compiles; (b) revisit the 9 tpssloc entries in tools/kernel_size_exceptions.txt via a size audit; (c) Plan 11.1-03 G4 full-649 f32 oracle is now unblocked for tpssloc COMPILATION (numeric parity still G3/G4's job); (d) apply the same hier+split recipe to the other D-LOCK-B candidates (gga_c_ft97, mgga_c_kcis/kcisk/revtpss/rmggac lxc_pol, lda_c_pk09 kxc_pol) if/when needed.
Resume file: .planning/quick/260520-eem-mgga-c-tpssloc-subcrate-split/260520-eem-SUMMARY.md
