...
status: Phase 11 RE-OPENED — Wave 1 PARTIAL (2026-05-22). 11-09 G-1 ✓ (canary PASS 1e-12); 11-11 G-4 ✓ (D-LOCK-D SATISFIED); 11-10 G-3 DEFERRED. ⚠ NEW BLOCKER found by 11-09 entry gate: the umbrella libxc_rs lib does NOT compile under cubecl 0.10 (systemic launch-ABI drift, 3031 errors in dispatch glue — NOT kernels, NOT the τ-clamp edit) → gates G-2/11-12; 11-10's kernel sweep won't catch it. ✓ RE-PLANNED 2026-05-22 (/gsd:plan-phase 11 --gaps): 11-14 (G-6 cubecl-0.10 launch-ABI migration) authored + plan-checker PASSED (no blockers); 11-12 rewired depends_on +11-14. Next: /gsd:execute-phase 11 --gaps-only --wave 1 (runs 11-14; 11-12 unblocks at wave 2 once umbrella compiles). Milestone v1.0 IN PROGRESS (Phases 5, 6, 7 + Phase 11 closure remain). [Phase 11.1 COMPLETE 2026-05-22.]
stopped_at: "Phase 11 Wave 1 executed inline-sequential (2026-05-22, /gsd:execute-phase 11 --gaps-only --wave 1). DONE: 11-09 G-1 — von Weizsäcker τ-clamp added to PRODUCTION src/eval/mgga_dispatch/prepare.rs + wired into dispatch_mgga step 5 + mirrored into tools/generate_mgga_dispatch.py (durable across regen); verify-canary g1 test PASS at 1e-12 on a sub-vW grid (commits 871f0b9976 feat, 1f3193e472 test, 160100a134 summary). 11-11 G-4 — D-LOCK-D idempotency SATISFIED: vanilla translate --family all → ZERO diff for the 264 non-sharded (incl. untracked cross-checks); tpssloc/revtpss Option A (deterministic split post-process; selftest double-run byte-identical + idempotency-guard no-op) (commits 8a2640a98c proof, b3ae549257 summary). NOT DONE: 11-10 G-3 DEFERRED by user decision. ⚠ BLOCKER (11-09 entry gate, log/libxc_rs_check.log): `cargo check -p libxc_rs --lib` = 3031 errors, ALL pre-existing cubecl-0.10 launch-ABI drift in the umbrella dispatch glue (launch_mgga! macro mod.rs:107-161, eval/{gga,mgga}_dispatch/funcs/*.rs, src/kernel/launch.rs, eval/dispatch.rs): stale from_raw_parts::<f64>(h,len,1)→0.10 wants from_raw_parts(h,len); launch_unchecked now returns (); ScalarArg removed. Kernel subcrates compile fine; the τ-clamp edit is clean (0 errors). Blocks G-2/11-12; 11-10's per-kernel sweep won't catch it. See memory project_umbrella_cubecl010_launch_abi_drift. NEXT: `/gsd:plan-phase 11 --gaps` to add a cubecl-0.10 launch-ABI migration plan (fix the dispatch generators tools/generate_*_dispatch.py + src/kernel/launch.rs to the 2-arg/no-turbofish/()-return ABI), THEN resume 11-10 + 11-12 + 11-13. HARD: inline sequential, jobs=1 (never edit .cargo/config.toml — it currently has jobs commented out/uncapped; user fixes by hand), NO monolithic umbrella build, verify-canary single-kernel escape hatch, per-`-p` compile as ENTRY gate."
last_updated: "2026-05-22T14:45:00.000Z"
...
**Current focus:** Phase 11 (re-opened) — gap closure. Wave 1 PARTIAL: 11-09 ✓ + 11-11 ✓; 11-10 deferred. 11-14 (G-6 umbrella launch-ABI migration) PLANNED + plan-checker PASSED. NEXT = /gsd:execute-phase 11 --gaps-only --wave 1 (run 11-14), then resume 11-10/11-12/11-13.

## Current Position

Phase: 11 (splitter-v2-unified-5k-cap) — RE-OPENED, Wave 1 PARTIAL (2026-05-22, executed inline-sequential via /gsd:execute-phase 11 --gaps-only --wave 1).
Wave 1 status: 11-09 G-1 ✓ DONE (production τ-clamp + canary PASS 1e-12); 11-11 G-4 ✓ DONE (D-LOCK-D SATISFIED, 264 zero-diff); 11-10 G-3 ○ DEFERRED (user chose to re-plan the umbrella blocker before the multi-day sweep). Remaining: 11-12 (G-2, W2, BLOCKED by umbrella drift), 11-13 (G-5, W3). Original 11-01..08 untouched (all have SUMMARYs).
⚠ NEW BLOCKER (11-09 entry gate): umbrella libxc_rs lib does NOT compile under cubecl 0.10 — 3031 launch-ABI-drift errors in the dispatch glue (NOT kernels, NOT the τ-clamp). Gates G-2/11-12; 11-10's kernel sweep won't catch it. Memory: project_umbrella_cubecl010_launch_abi_drift. ✓ NOW PLANNED as G-6 → 11-14-PLAN.md (3 tasks: ABI-confirm checkpoint + fix generators tools/generate_{gga,mgga}_dispatch.py & hand-written src/eval/dispatch.rs + src/kernel/launch.rs then regen + `cargo check -p libxc_rs --lib` exit-0 gate at jobs=1 with peak-RSS capture). 11-14 depends_on []; 11-12 rewired depends_on [11-09, 11-14]. plan-checker PASSED (commit 407fbe3c77; gap index abd258b1b2). Next: /gsd:execute-phase 11 --gaps-only --wave 1.
