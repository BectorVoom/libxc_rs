...
### Splitter emission target (LOCKED MID-PLANNING — 2026-05-13 user revision)
- **D-10 (splitter emits unified):** `tools/translate_{lda_v2,gga,mgga}.py` and all helpers emit DIRECTLY into `crates/kernels/{lda,gga,mgga}/src/<func>/...` — never into numbered subcrates. **The splitter does not know the numbered subcrate layout.** Per-family `lib.rs` enumerates `pub mod <func>;` per functional (alphabetically sorted for splitter-emit determinism). This locks the END state from the start: no transitional `crates/kernels/{family}-N/` directories ever exist post-Phase-11. Supersedes the original Phase-11 plan (committed as 11-02..06 first draft) which regenerated INTO numbered subcrates and then collapsed them in a final wave; that two-step is rejected in favor of one-step direct emission.
- **D-10a (clean slate):** The 22 existing numbered subcrates (`lda-1`, `lda-2`, `gga-1..8`, `mgga-1..7, 8a, 8b, 9a, 9b, 10, 11a, 11b, 12, 13, 14`) are DELETED UPFRONT in the first substantive wave (new plan 11-02), along with their entries in root `Cargo.toml` `[default-members]` array AND the root crate's `[dependencies]` block (`libxc-kernel-lda-{1,2}`, `libxc-kernel-mgga-{1..14 with letter suffixes}` — 19 path-deps total under `[dependencies]` today; GGA numbered children are NOT in `[dependencies]` per the actual root Cargo.toml, only in `[workspace.default-members]`). The splitter then populates the unified family crates from scratch. There is NO state where both layouts coexist. Risk acknowledged: if mid-flight regen fails, the working tree is half-broken until rollback (mitigated by per-wave atomic commits and the `cargo build -p libxc-kernel-{family}` per-family verification gate, plus `git checkout -- .` rollback path).
- **D-10b (Strategy 1 obsolete):** RESEARCH.md "Strategy 1 (façade preserves dispatch paths)" is REJECTED in favor of D-10. The original Strategy 1 envisioned numbered subcrates surviving as `pub mod batchN;` re-exports inside the family façade — D-10 forbids this. The per-family façade re-exports per-functional (`pub mod lda_x;`, `pub mod lda_c_pw;`, ...), not per-batch. The dispatch tree IS regenerated against this new shape (new plan 11-02): `src/eval/{gga,mgga}_dispatch/*.rs` paths become `crate::kernel::{family}::<func>::...` (no `batchN::` segment). The dispatch generator (`tools/generate_gga_dispatch.py` and the MGGA equivalent — to be added if absent) and its input rosters (`.planning/phases/04-bulk-kernel-translation/{gga,mgga}_roster.tsv`) are updated accordingly: the `batch` column either stays as a degenerate constant (e.g., always "0" or empty) or the generator stops emitting per-batch submodules entirely. Planner's call which form is used (recommended: drop the per-batch submodule layer, emit one `dispatch_<func>` helper per functional under `src/eval/{family}_dispatch/funcs/<func>.rs`).
...
- `.cargo/config.toml` — **load-bearing build environment**. `[build] jobs = 1`, `target-dir = .cache/cargo-target`, `[env] RUST_MIN_STACK = 67108864`. See D-07/D-08/D-09 for the rules. 
