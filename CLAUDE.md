...
| Bypassing the screen *or* the input clamps changes the answer | HIGH | libxc's `work_{lda,gga,mgga}_inc.c` both screens below-threshold points and **sanitises** the inputs it keeps -- `m_max(dens_threshold, rho)`, `m_max(sigma_threshold^2, sigma)`, `m_max(tau_threshold, tau)`, the `+-s_ave` bound on the cross term, and the Fermi-hole curvature clamp `sigma <- min(sigma, 8 rho tau)` that libxc's own CMake enables by default. Only the screen was implemented until 2026-09-10; the missing clamps were `mgga_c_r2scan`'s `vtau` (9e-8) and `mgga_x_scan`'s polarized `vsigma` (3.5e-9). `crate::screen` + the generated `sweep_*.rs` carry both halves, and the fast-path predicate *is* the transform (`needs_sanitise` runs `sanitise_point` and compares bitwise), so the two cannot drift apart. See AGENTS.md, "Screening and sanitisation". |
| Thresholds are per functional, not global | HIGH | `xc_func_init` seeds `dens_threshold` from `info->dens_threshold` (1e-15 for 432 of 649, but 1e-14 for 113, 1e-12 for 40, down to 1e-32) and derives `sigma_threshold = pow(dens_threshold, 4/3)`. A single global default gave **217 functionals** a different screen *and* different clamps than libxc. Use `Thresholds::for_functional(id)`; `FunctionalMeta::default_density_threshold` has carried the right value all along. Each auxiliary of a mix keeps its own, which is why the fused HSE06/PBE0 kernels take one `dens_threshold` per leg. |
| A root-finder's stopping rule is part of its answer | HIGH | `br89`, `mbrxc` and `lambert_w` were CubeCL-era fixed-iteration transcriptions with no convergence test. Brent keeps moving inside the tolerance, so 60 steps lands elsewhere than libxc's "return `(a+b)/2` when `\|b-a\| < TOL`" -- and BR89's `TOL` is *absolute*, so at a root of 3e-8 it pins four digits. Now transcribed as loops (`math/src/brent.rs`) and gated by `verify/tests/root_finders.rs` against libxc's own exported C, not through a functional. |
| C libxc leaking into the published crate | MEDIUM | The package directory is the repo root, so `cargo package` would otherwise ship the 372 MB vendored `libxc-master/`, the bench and the verify suite -- and fail outright on 22,654 `:Zone.Identifier` filenames. Held apart by three things that do different jobs: a `[package] include` **allowlist** (decides the tarball: 11 files), opt-in `libxc-oracle` features on `bench-vs-libxc`/`xtask` (decides whether the C is *compiled* at all), and `publish = false` on every harness. `cargo package --list -p libxc_rs` is the check. See AGENTS.md, "What ships". |
