...
**CPU only.** The CubeCL substrate that previously provided single-source CPU+GPU execution was retired in ADR 0001 (`docs/adr/0001-rayon-over-cubecl.md`) after it measured 2-3x slower than plain rayon on the CPU and imposed build costs that made several MGGA functionals uncompilable. Its archived kernel tree was deleted on 2026-08-18; kernels are now generated straight from libxc's maple2c C by `tools/translate_rayon/from_maple.py`. There is no GPU path. CubeCL remains only as an optional, default-off feature of `crates/kernels-rayon/math` (`Line<F>` primitives for a future vectorised path).
...
- **f32 support**: not a target. The kernels are f64-concrete by design — 2,892 emitted files over `&[f64]`, generated directly from libxc's maple2c C. Supporting f32 would mean re-architecting the translator to emit float-generic kernels, a full regen, and reconciling FP order. The CubeCL-era note about `&Array<f64>` no longer applies.
- **Dependencies**: rayon 1.11, wide 1.6 (explicit SIMD, pure Rust), thiserror 2.0, bitflags 2.10, bytemuck 1.25 (production); bindgen, anyhow, criterion (verification/benchmark only)
