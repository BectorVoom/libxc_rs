...
pub fn gga_k_tflw_kxc_unpol(
    ...
    if ip < zk.len() {
        ...
        let t44 = piecewise3::<f64>(t2, 0.0, 3.0 / 20.0 * t7 * t20 * t22 * t40);
        let tzk0 = 2.0 * t44;
        zk[ip] += tzk0;
        let t50 = t28 * rho[ip];
        let t53 = t7 * t20 / t50;
        let t56 = t24 * t26 * t31 * t36;
        let t60 = piecewise3::<f64>(t2, 0.0, t7 * t20 / t21 * t40 / 10.0 - t53 * t56 / 36.0);
        let tvrho0 = 2.0 * rho[ip] * t60 + 2.0 * t44;
        vrho[ip] += tvrho0;
        let t68 = param_lambda * t26 * t31 * t36;
        let t71 = piecewise3::<f64>(t2, 0.0, t7 * t20 / t28 * t68 / 96.0);
        let tvsigma0 = 2.0 * rho[ip] * t71;
        vsigma[ip] += tvsigma0;
        let t80 = t28 * t28;
        let t83 = t7 * t20 / t80;
        let t87 = piecewise3::<f64>(t2, 0.0, -t7 * t20 / t21 / rho[ip] * t40 / 30.0 + 7.0 / 108.0 * t83 * t56);
        let tv2rho20 = 2.0 * rho[ip] * t87 + 4.0 * t60;
        v2rho2[ip] += tv2rho20;
        let t92 = piecewise3::<f64>(t2, 0.0, -t53 * t68 / 48.0);
        let tv2rhosigma0 = 2.0 * rho[ip] * t92 + 2.0 * t71;
        v2rhosigma[ip] += tv2rhosigma0;
        let tv2sigma20 = 0.0;
        v2sigma2[ip] += tv2sigma20;
        let t105 = t7 * t20 / t80 / rho[ip];
        let t109 = piecewise3::<f64>(t2, 0.0, 2.0 / 45.0 * t7 * t20 / t21 / t28 * t40 - 41.0 / 162.0 * t105 * t56);
        let tv3rho30 = 2.0 * rho[ip] * t109 + 6.0 * t87;
        v3rho3[ip] += tv3rho30;
        let t115 = piecewise3::<f64>(t2, 0.0, t83 * t68 / 16.0);
