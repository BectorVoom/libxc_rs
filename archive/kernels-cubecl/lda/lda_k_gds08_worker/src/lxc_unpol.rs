//! LDA_K_GDS08_WORKER lxc unpol kernel.
//!
...
pub fn lda_k_gds08_worker_lxc_unpol(
    ...
    if ip < zk.len() {
        ...
        let t30 = 1.0 / t29;
        let t39 = piecewise3::<f64>(t2, 0.0, t7 * (-2.0 * t19 * t30 - param_B * t30 + 2.0 * param_C * t30) / 2.0);
        let tv2rho20 = 2.0 * rho[ip] * t39 + 4.0 * t25;
        v2rho2[ip] += tv2rho20;
        let t44 = 1.0 / t29 / rho[ip];
        let t54 = piecewise3::<f64>(t2, 0.0, t7 * (4.0 * t19 * t44 + 2.0 * param_B * t44 - 6.0 * param_C * t44) / 2.0);
        let tv3rho30 = 2.0 * rho[ip] * t54 + 6.0 * t39;
        v3rho3[ip] += tv3rho30;
        let t58 = t29 * t29;
        let t59 = 1.0 / t58;
        let t69 = piecewise3::<f64>(t2, 0.0, t7 * (-12.0 * t19 * t59 - 6.0 * param_B * t59 + 22.0 * param_C * t59) / 2.0);
