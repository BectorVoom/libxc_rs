...
pub fn mgga_k_gea2_fxc_unpol(
    ...
    if ip < zk.len() {
        ...
        let t50 = piecewise3::<f64>(t3, 0.0, 3.0 / 20.0 * t8 * t24 * t46);
        let tzk0 = 2.0 * t50;
        zk[ip] += tzk0;
        let t52 = t21 / t22;
        let t56 = t34 * rho[ip];
        ...
        let t70 = piecewise3::<f64>(t3, 0.0, t8 * t52 * t46 / 10.0 + 3.0 / 20.0 * t8 * t24 * t65);
        let tvrho0 = 2.0 * rho[ip] * t70 + 2.0 * t50;
        vrho[ip] += tvrho0;
        let t73 = t8 * t21;
        let t76 = t29 * t32;
        let t78 = t73 / t34 * t25 * t76;
        let t80 = piecewise3::<f64>(t3, 0.0, t78 / 864.0);
        let tvsigma0 = 2.0 * rho[ip] * t80;
        vsigma[ip] += tvsigma0;
        let t87 = piecewise3::<f64>(t3, 0.0, t73 / rho[ip] * t25 * t76 / 72.0);
        ...
        let t113 = piecewise3::<f64>(t3, 0.0, -t8 * t92 * t46 / 30.0 + t8 * t52 * t65 / 5.0 + 3.0 / 20.0 * t8 * t24 * t108);
        let tv2rho20 = 2.0 * rho[ip] * t113 + 4.0 * t70;
        v2rho2[ip] += tv2rho20;
        let t119 = t73 / t56 * t25 * t76;
        let t121 = piecewise3::<f64>(t3, 0.0, -t119 / 432.0);
        let tv2rhosigma0 = 2.0 * rho[ip] * t121 + 2.0 * t80;
        v2rhosigma[ip] += tv2rhosigma0;
        let t125 = piecewise3::<f64>(t3, 0.0, -t78 / 72.0);
