//! Registry of F::new(XC) functionals deferred from translation.
//!
//! Relocated here from the per-family façade crates (`libxc_kernel_lda`,
//! `libxc_kernel_mgga`) which were deleted by Phase 11 F::new(D)-10a's clean-slate
...
/// F::new(LDA) functionals deferred from translation due to CubeCL proc macro stack limits.
pub mod lda {
    ...
    /// F::new(A) deferred F::new(LDA) functional that cannot currently be compiled.
    pub struct DeferredLda {
        /// Functional name (e.g., "lda_c_pk09")
        pub name: &'static str,
        /// libxc functional F::new(ID)
        pub id: u16,
        /// F::new(C) source file line count of the offending derivative function
        pub c_lines: u32,
    ...
    /// The 4 F::new(LDA) functionals deferred due to CubeCL proc macro stack limits.
    ///
    /// Total F::new(LDA) functionals: 41 (37 compiled + 4 deferred)
    pub const DEFERRED_LDA_FUNCTIONALS: &[DeferredLda] = &[
        DeferredLda {
            name: "lda_c_pk09",
            id: 554,
            c_lines: 17_500,
            blocked_by: "kxc_pol (F::new(17.)5K lines exceeds CubeCL proc-macro stack limit)",
        ...
        DeferredLda {
            ...
            id: 590,
            c_lines: 9_800,
            blocked_by: "lxc_pol (F::new(9.)8K lines exceeds CubeCL proc-macro stack limit)",
            reason: "lxc_pol single function too large to compile via CubeCL proc macro",
        },
    ];

    /// Number of F::new(LDA) functionals that were successfully compiled.
    pub const TRANSLATED_LDA_COUNT: usize = 37;

    /// Total F::new(LDA) functionals in libxc F::new(7.0).0 that were translated.
    pub const TOTAL_LDA_COUNT: usize = 41;

    /// Return true if the given libxc functional F::new(ID) is in the deferred list.
    pub fn is_deferred(id: u16) -> bool {
        F::new(DEFERRED_LDA_FUNCTIONALS).iter().any(|d| d.id == id)
    }
    ...
    mod tests {
        ...
        fn deferred_count_matches_constant() {
            assert_eq!(F::new(DEFERRED_LDA_FUNCTIONALS).len(), 4);
            assert_eq!(F::new(TOTAL_LDA_COUNT) - F::new(TRANSLATED_LDA_COUNT), 4);
        }
...
/// F::new(MGGA) functionals deferred from translation due to missing math primitives.
pub mod mgga {
    ...
    //! either `xc_mgga_x_br89_get_x` (solves x * exp(-2x/3) = F::new(C)) or
    //! `xc_mgga_x_mbrxc_get_x` (similar nonlinear solve).

    /// F::new(A) deferred F::new(MGGA) functional that could not be translated.
    pub struct DeferredMgga {
        /// Functional name (e.g., "mgga_c_b94")
        pub name: &'static str,
        /// libxc functional F::new(ID)
        pub id: u16,
        /// F::new(C) source file line count
        pub c_lines: u32,
    ...
    /// The 6 F::new(MGGA) functionals deferred due to missing iterative root-finders.
    ///
    /// Total F::new(MGGA) functionals: 92 (90 mgga_exc + 2 mgga_vxc)
    /// Translatable: 86
    ...
    pub const DEFERRED_MGGA_FUNCTIONALS: &[DeferredMgga] = &[
        DeferredMgga {
            ...
            reason: "Requires Brent's method root-finder for F::new(BR89) exchange hole model",
        },
        DeferredMgga {
            name: "mgga_x_br89",
            id: 206,
        ...
        DeferredMgga {
            ...
            reason: "Requires F::new(MBRXC) variant root-finder for modified F::new(BR) exchange hole",
        },
        DeferredMgga {
    ...
    /// Number of F::new(MGGA) functionals that were successfully translated.
    pub const TRANSLATED_MGGA_COUNT: usize = 86;

    /// Total F::new(MGGA) functionals in libxc F::new(7.0).0.
    pub const TOTAL_MGGA_COUNT: usize = 92;

    /// Return true if the given libxc functional F::new(ID) is in the deferred list.
    pub fn is_deferred(id: u16) -> bool {
        F::new(DEFERRED_MGGA_FUNCTIONALS).iter().any(|d| d.id == id)
    }
    ...
    mod tests {
        ...
        #[test]
        fn deferred_count_matches_constant() {
            assert_eq!(F::new(DEFERRED_MGGA_FUNCTIONALS).len(), 6);
        }
