//! Per-batch dispatch layer for MGGA kernel evaluation.
//!
//! Routes `dispatch_mgga(functional, ...)` to per-batch launch helpers
//! mirroring the 95 `crates/kernel-mgga-*` sub-crates. The outer match in
...
//! pair), dispatch wires the unpolarized `Exc` and `Vxc` launches directly.
//! Kernels that take per-functional scalars (e.g. `mgga_x_tpss` with 7
//! params, `mgga_x_task` with 11 params) are listed in the `from_id` match
...
//! `exc_unpol`/`exc_pol` modules; its signatures omit the `zk` output
//! array. Dispatch rejects `DerivativeOrder::Exc` for this variant, then
//! routes to the vxc-only launch shape (currently also deferred pending
//! param wiring — it has 2 per-functional scalars).
//!
//! **Derivative order scope:** This plan wires `Exc` and `Vxc` arms for
//! unpolarized spin only (what oracle parity requires for the zero-scalar
//! subset). `Fxc`/`Kxc`/`Lxc` orders and polarized spin mode return
//! `UnsupportedDerivativeOrder` / `UnsupportedFunctional` pending Phase 4
//! follow-up — the MGGA output surface (70 fields at lxc) and polarized-
//! kernel translation bugs (same ones seen in GGA plan 04-03's deferred
//! D-04-03-A) make a complete wiring impractical in this plan's scope.
//! All 25 routable variants are fully listed so the dispatch surface is
...
/// the Exc and Vxc arms that this plan actually launches.
...
/// are also skipped by this macro — callers ask for polarized mode
...
/// (B3: no shared `MggaFunctionalParams` struct). If a functional's kernel
...
///   functional like `MggaXTb09`, or for orders >= Fxc (this plan
...
pub fn dispatch_mgga(
    ...
    // 3. Reject high-order dispatch early: this plan wires only Exc and Vxc arms
    // for the MGGA family. Fxc/Kxc/Lxc are scoped to a Phase 4 follow-up plan
    // given the 70-output-field × 25-functional × 2-spin surface.
    ...
    match functional {
        MggaFunctional::HybMggaXDldf   => batch21::dispatch_hyb_mgga_x_dldf(&ctx, order, spin)?,
        MggaFunctional::MggaXcZlp      => batch23::dispatch_mgga_xc_zlp(&ctx, order, spin)?,
        MggaFunctional::MggaCCs        => batch34::dispatch_mgga_c_cs(&ctx, order, spin)?,
        MggaFunctional::MggaXLta       => batch34::dispatch_mgga_x_lta(&ctx, order, spin)?,
        MggaFunctional::MggaXTpss      => batch33::dispatch_mgga_x_tpss(&ctx, order, spin)?,
        MggaFunctional::MggaXTauHcth   => batch34::dispatch_mgga_x_tau_hcth(&ctx, order, spin)?,
        MggaFunctional::MggaXTb09      => batch35::dispatch_mgga_x_tb09(&ctx, order, spin)?,
        MggaFunctional::MggaXPkzb      => batch28::dispatch_mgga_x_pkzb(&ctx, order, spin)?,
        MggaFunctional::MggaXTh        => batch34::dispatch_mgga_x_th(&ctx, order, spin)?,
        MggaFunctional::MggaXcCc06     => batch35::dispatch_mgga_xc_cc06(&ctx, order, spin)?,
        MggaFunctional::MggaXJk        => batch35::dispatch_mgga_x_jk(&ctx, order, spin)?,
        MggaFunctional::MggaXMvs       => batch35::dispatch_mgga_x_mvs(&ctx, order, spin)?,
        MggaFunctional::MggaXRtpss     => batch21::dispatch_mgga_x_rtpss(&ctx, order, spin)?,
        MggaFunctional::MggaCCc        => batch35::dispatch_mgga_c_cc(&ctx, order, spin)?,
        MggaFunctional::HybMggaXM05    => batch29::dispatch_hyb_mgga_x_m05(&ctx, order, spin)?,
        MggaFunctional::MggaXTm        => batch30::dispatch_mgga_x_tm(&ctx, order, spin)?,
        MggaFunctional::MggaXcLp90     => batch29::dispatch_mgga_xc_lp90(&ctx, order, spin)?,
        MggaFunctional::MggaXGx        => batch29::dispatch_mgga_x_gx(&ctx, order, spin)?,
        MggaFunctional::MggaXPbeGx     => batch29::dispatch_mgga_x_pbe_gx(&ctx, order, spin)?,
        MggaFunctional::MggaX2dJs17    => batch34::dispatch_mgga_x_2d_js17(&ctx, order, spin)?,
        MggaFunctional::MggaKRda       => batch29::dispatch_mgga_k_rda(&ctx, order, spin)?,
        MggaFunctional::MggaKGea2      => batch17::dispatch_mgga_k_gea2(&ctx, order, spin)?,
        MggaFunctional::MggaKGea4      => batch34::dispatch_mgga_k_gea4(&ctx, order, spin)?,
        MggaFunctional::MggaXRlda      => batch34::dispatch_mgga_x_rlda(&ctx, order, spin)?,
        MggaFunctional::MggaXTask      => batch33::dispatch_mgga_x_task(&ctx, order, spin)?,
    }
...
mod tests {
    ...
    fn dispatch_mgga_rejects_fxc_scope() {
        // This plan only wires Exc+Vxc arms. Fxc must return
