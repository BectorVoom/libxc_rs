...
# functionals that `GgaFunctional::from_id` does not route, so regen does
...
def detect_imports(c_src: str) -> list:
    ...
    # Phase 11.1 (2026-05-19): detect Bessel imports defensively (gga_x_am05
    # uses lambert_w only; future GGA functionals may pull I/K Bessel too).
...
def translate_line(line: str, is_pol: bool) -> str:
    ...
    # concrete-f64 and Rule 9 concrete-caller form requires `::<f64>` against
    # a generic <F: Float> callee. The CHUNKED emission path retargets
    # `::<f64>` -> `::<F>` via tools/translate_v2/per_functional.py::
    # _wrap_f64_literals_v2's _FN_F64_TURBOFISH_RE retargeting pass (D-08
    # native chunked emit). See PATTERN.md Rule 10 § "SUPERSEDED 2026-05-19".
    ...
    for fn in [
        ...
        'lambert_w',                                     # lambert_w.rs (added 2026-05-19 Phase 11.1; used by gga_x_am05)
...
def emit_per_functional(c_file: str, func_name: str, family: str = 'gga',
    ...
    adapter = per_functional.FamilyAdapter(
        ...
        src_dir_label='gga_vxc' if is_vxc_only else 'gga_exc',
        ...
        build_dep_graph=lambda c, is_pol: build_dependency_graph(c, is_pol)[:2],
