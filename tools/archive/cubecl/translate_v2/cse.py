...
def partition_compute_lines(compute_lines, var_deps, est_line_fn, *,
    ...
    while i < n:
        ...
        # Struct mode (natural_breakpoints=False) cuts on size only: the
        # reverse-dep heuristic exists to find LOW-ARITY tuple cut points,
        # which a struct interface does not need — left on, it fragments a
        # wide-dataflow output into hundreds of tiny chunks.
        natural = False
        if natural_breakpoints and var is not None and not is_bool:
        ...
        # Chunk-first struct mode passes max_tuple_arity=None: struct-return
        # interfaces have no 12-field ceiling, so ranges cut on size/natural
        # breakpoints only (D-02 tuple mode keeps the default 12).
        if max_tuple_arity is None:
            arity_forced = False
        else:
            arity_forced = (inputs_arity >= max_tuple_arity
                            or outputs_open >= max_tuple_arity)
        # Struct mode: fn params are unbounded, but the RETURN struct's field
        # count is capped — cubecl-cpu's lowering overflows its worker stack
        # (even at 2 GB) once a returned struct reaches ~900 fields on a
        # non-trivial body; <= 700 is measured-safe, 512 is the margin pick.
        if max_out_arity is not None and outputs_open >= max_out_arity:
            arity_forced = True
