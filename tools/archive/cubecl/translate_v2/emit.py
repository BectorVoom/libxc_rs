...
# --- Incremental-sync session state (build-time optimization) -----------------
# Rationale: the old flow wiped `<func>/src/` and rewrote every file on each
# regen, which bumped every mtime and invalidated cargo's fingerprint for all
# ~650 kernel subcrates — a full-workspace rebuild after every regen, even a
# no-op one. The sync model keeps byte-identical files UNTOUCHED (mtime
# preserved -> cargo skips the crate), writes only changed/new files, and
# deletes stale files at finalize (preserving the 11.1-02-fix5 E0761 guarantee
# without the wipe). Generated bytes are identical to the wipe flow — only
# filesystem effects differ.
_session_root: Path | None = None
_session_files: set[Path] | None = None
_stats = {"written": 0, "unchanged": 0, "deleted": 0}
...
def _write(path: Path, content: str) -> None:
    ...
    Incremental-sync: when the on-disk file already holds exactly the target
    bytes, the write is SKIPPED so the mtime survives and cargo's fingerprint
    for the owning subcrate stays valid (no rebuild). Every call is recorded in
    the active subcrate session so finalize_subcrate can delete stale files."""
    body = content.rstrip("\n") + "\n"
    body = "\n".join(line.rstrip() for line in body.split("\n"))
    if _session_files is not None:
        _session_files.add(path.resolve())
    try:
        if path.is_file() and path.read_text(encoding="utf-8") == body:
            _stats["unchanged"] += 1
            return
    except (OSError, UnicodeDecodeError):
        pass  # unreadable/undecodable -> just rewrite it
    # Layout shifts across regens (flat `partN.rs` <-> dir `partN/mod.rs`):
    # without the old pre-wipe, a directory may sit where a file is now needed,
    # or a stale file may block a needed parent directory. Clear both.
    if path.is_dir():
        shutil.rmtree(path)
    parent = path.parent
...
def clean_subcrate_src(family: str, func: str) -> None:
    ...
    Incremental-sync rework: this no longer wipes src/ up front (a wipe
    forces every file's mtime forward and triggers a full cargo rebuild of
    the subcrate even when nothing changed). Instead it OPENS a write-
    recording session; ``finalize_subcrate`` closes it by deleting every
    file under the subcrate that this regen did not write — which removes
    the same stale ``partN.rs`` / ``partN/mod.rs`` E0761 collisions the old
    wipe removed, without touching byte-identical survivors.
    """
    global _session_root, _session_files
    _session_root = subcrate_dir(family, func).resolve()
    _session_files = set()


def finalize_subcrate(family: str, func: str) -> None:
    """Close the incremental-sync session opened by ``clean_subcrate_src``:
    delete every file under ``<func>/`` that the session did not write
    (stale parts from a prior regen with a different split layout), then
    prune empty directories. No-op when no session is active (direct calls
    to individual emit_* helpers, e.g. the selftest)."""
    global _session_root, _session_files
    d = subcrate_dir(family, func).resolve()
    if _session_files is None or _session_root != d:
        _session_root = None
        _session_files = None
        return
    if d.exists():
        for p in sorted(d.rglob("*"), reverse=True):
            if p.is_file() and p.resolve() not in _session_files:
                p.unlink()
                _stats["deleted"] += 1
...
def emit_chunk_pipeline_output(family: str, func: str, output: str,
                               wrapper_src: str, chunk_srcs: list) -> None:
    """Write a chunk-first output: ``<func>/src/<output>/mod.rs`` (the entry
    wrapper — carries the routing ``#[cube]``/``#[cube(launch_unchecked)]``
    attribute and the canonical entry ABI) plus one
    ``<func>/src/<output>/chunk{n}.rs`` per D-02 CSE chunk.

    Chunk-first (2026-08-15): the whole output is ONE sequential tuple-return
    chunk pipeline instead of independent per-component parts, so no chunk
    recomputes another's values (the per-component layout duplicated 50-92% of
    all let-defs — see memory project-variable-recomputation-analysis).
    chunk{n} indices are 0-based sequence order (D-LOCK-D determinism)."""
    out_dir = subcrate_dir(family, func) / "src" / output
    _write(out_dir / "mod.rs", wrapper_src)
    for nn, chunk_src in enumerate(chunk_srcs):
        _write(out_dir / f"chunk{nn}.rs", chunk_src)
