//! Phase 12 Plan 02 — single-kernel f64 parity canary for `mgga_x_pkzb` (libxc id 213).
//!
//! Sibling of `g3_mgga_c_b94_parity.rs`. The kernel is ROUTED
//! (`mgga_x_pkzb_exc_unpol` is `#[cube(launch_unchecked)]`), so it is launched
...
use libxc_kernel_mgga_x_pkzb::exc_unpol::mgga_x_pkzb_exc_unpol;
...
/// Evaluate `mgga_x_pkzb_exc_unpol` on the regularized grid via direct CubeCL launch.
fn rust_zk() -> Vec<f64> {
    let np = RHO.len();
    ...
    unsafe {
        mgga_x_pkzb_exc_unpol::launch_unchecked::<CpuRuntime>(
            &client,
            cube_count,
            cube_dim,
...
/// Evaluate libxc id=213 exc, unpolarized, on the RAW grid (libxc regularizes internally).
...
#[test]
fn mgga_x_pkzb_f64_parity_1e12() {
    let np = RHO.len();
    ...
    for i in 0..np {
        ...
        eprintln!(
            "PARITY_POINT: mgga_x_pkzb exc unpol zk[{i}] rust={:.15e} c={:.15e} rel_err={:.3e}",
            rust[i], oracle[i], e
        );
        if e > STRICT_TOL {
    ...
    eprintln!(
        "PARITY_SUMMARY: mgga_x_pkzb exc unpol np={np} max_rel_err={max_e:.3e} \
    ...
    assert!(
        failures.is_empty(),
        "mgga_x_pkzb f64 parity FAILED at strict 1e-12 ({} point(s)):\n  {}",
        failures.len(),
        failures.join("\n  ")
    );
}
