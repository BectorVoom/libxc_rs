...
//!
//! For each functional id from `libxc_rs::registry::all_functional_ids()`:
//!   1. Allocate a zeroed `xc_func_type`.
//!   2. Call `xc_func_init(...)`. If rc != 0, bail.
//!   3. Read `xc_func_info_type` fields (name, kind, family, flags, refs,
...
//!      classification.
//!   4. Build a `Snapshot` struct mirroring `FunctionalMeta` shape.
//!   5. Call `xc_func_end`.
//!
...
pub fn run() -> Result<()> {
    ...
    if snapshots.is_empty() {
        bail!("No functionals collected — registry::all_functional_ids() returned empty");
    }
...
fn collect_all_functionals() -> Result<Vec<Snapshot>> {
    ...
    for id in libxc_rs::registry::all_functional_ids() {
        let raw = id.raw();
        let canonical_name = id.name().to_string();
...
fn write_generated_propagation_rs(root: &Path, snapshots: &[Snapshot]) -> Result<()> {
    ...
    //   (parent_name, parent_param_name, parent_param_index, aux_slot, aux_param_name)
    // where parent_name resolves via the by_name map and aux_param_name is
    // the named ext_param on the aux at `aux_slot`.
    //
    // Sources:
    ...
    //
    // Note: parent_param_index is the index in parent.ext_params; a probe
    // step would have to look it up from snapshots to be fully data-driven.
    // For the canonical hybrids whose first ext_param IS `_omega`, index=0.
    let curated: &[(&str, &str, u16, u8, &str)] = &[
        ...
        ("XC_HYB_GGA_XC_CAM_B3LYP", "_omega", 0, 1, "_omega"),
        ("XC_HYB_GGA_XC_CAMY_B3LYP", "_omega", 0, 1, "_omega"),
        ("XC_HYB_GGA_XC_CAM_O3LYP", "_omega", 0, 1, "_omega"),
        ("XC_HYB_GGA_XC_CAMH_B3LYP", "_omega", 0, 1, "_omega"),
        ("XC_HYB_GGA_XC_CAMY_PBEH", "_omega", 0, 1, "_omega"),
        ("XC_HYB_GGA_XC_CAM_QTP_00", "_omega", 0, 1, "_omega"),
        ("XC_HYB_GGA_XC_CAM_QTP_01", "_omega", 0, 1, "_omega"),
        ("XC_HYB_GGA_XC_CAM_QTP_02", "_omega", 0, 1, "_omega"),
        // HSE family: _omega -> func_aux[1] _omega
        ("XC_HYB_GGA_XC_HSE03", "_omega", 0, 1, "_omega"),
        ("XC_HYB_GGA_XC_HSE06", "_omega", 0, 1, "_omega"),
        ("XC_HYB_GGA_XC_HSE12", "_omega", 0, 1, "_omega"),
        ("XC_HYB_GGA_XC_HSE12S", "_omega", 0, 1, "_omega"),
        // wB97 family
        ("XC_HYB_GGA_XC_WB97", "_omega", 0, 1, "_omega"),
        ("XC_HYB_GGA_XC_WB97X", "_omega", 0, 1, "_omega"),
        ("XC_HYB_GGA_XC_WB97X_V", "_omega", 0, 1, "_omega"),
        ("XC_HYB_GGA_XC_WB97X_D3", "_omega", 0, 1, "_omega"),
        // LC-wPBE family
        ("XC_HYB_GGA_XC_LC_WPBE", "_omega", 0, 1, "_omega"),
        ("XC_HYB_GGA_XC_LC_WPBEH", "_omega", 0, 1, "_omega"),
    ];

    let mut emitted = 0usize;
    let mut skipped = 0usize;
    for (parent, pname, pidx, aux_slot, aname) in curated {
        if let Some(&pid) = by_name.get(*parent) {
            // Validate: parent must have a matching ext_param at the given
            // index, and the aux at aux_slot must exist in this snapshot.
            let parent_snap = snapshots.iter().find(|s| s.id == pid);
            let valid_parent = parent_snap
                .and_then(|s| s.ext_params.get(*pidx as usize))
                .map(|p| p.name == *pname)
                .unwrap_or(false);
