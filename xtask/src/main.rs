...
fn generate_meta_file(root: &Path, entries: &BTreeMap<u16, FuncEntry>) -> Result<()> {
    let path = root.join("crates/libxc-core/src/meta/generated.rs");
...
fn generate_by_id_file(root: &Path, entries: &BTreeMap<u16, FuncEntry>) -> Result<()> {
    let path = root.join("crates/libxc-core/src/registry/by_id.rs");
...
fn generate_by_name_file(root: &Path, entries: &BTreeMap<u16, FuncEntry>) -> Result<()> {
    let path = root.join("crates/libxc-core/src/registry/by_name.rs");
...
fn generate_removed_file(
    ...
    let path = root.join("crates/libxc-core/src/registry/removed.rs");
